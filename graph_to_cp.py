      ...
      if node_name in graph:
        ...
        for adj_node in graph[node_name]:
          ...
          if adj_node in no_op_nodes:
            ...
            edges_costs[node_name][adj_node] = int(float(splits[1]) * comm_transfer_rate_rec + comm_latency)
            edges_costs[adj_node][node_name] = comm_latency

with open(metis_input_file_name, 'w') as f:
  f.write(str(number_of_nodes) + ' ' + str(number_of_edgse) + ' 11\n')
  for node in graph.keys():
    line_to_write = str(nodes_weights[node]) if node in nodes_weights else '1'
...
with open(output2_file_name, 'w') as f:
  f.write("Id,(Outgoing) node,(Incoming) node,Colocation nodes,#tensorSize,#operations,RAM storage,Device constraint,name\n")
