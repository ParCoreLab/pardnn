...
class Node {
    public:
        ...
        Node();
        Node(string);
        Node(string, int, long long, int, int, int, int, int, int, int, int,
                NODE_TYPES);
    ...
    private:
        int duration, level, reversed_level, start_time, end_time, part, top_level,
            bottom_level, weighted_level;
        long long memory;
        NODE_TYPES type;
        string name;
};
