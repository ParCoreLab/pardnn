...
def train(hparams, scope=None, target_session=""):
    ...
    operations_tensors = {}
    operations_names = train_model.graph.get_operations()
    count1 = 0
    count2 = 0

    for operation in operations_names:
        operation_name = operation.name
        operations_info = train_model.graph.get_operation_by_name(operation_name).values()
    ...
    while global_step < num_train_steps:
        ...
        try:
            step_result = loaded_train_model.train(train_sess, options = options, run_metadata = run_metadata, eppoch = eppoch - 1)
            if eppoch % 7 == 0:
