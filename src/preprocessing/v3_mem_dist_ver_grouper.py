...
comm_transfer_rate = 1000000 / (25 * 1024 * 1024 * 1024)
...
while improvement_achieved and iter < iterations_threshold:
    ...
    # $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    with open(io_folder_path + 'nodes_levels.txt', 'w') as f:
      for node_ in all_nodes:
        f.write(node_ + ' ' + str(int(nodes_weighted_levels_t[node_] + nodes_weighted_levels_b[node_])) + '\n')
...
for node, mem in nodes_res_memory.items():
  wanted = False
  if node in graph:# and 'adam' not in node:
    wanted = True
    for adj_node in graph[node]:
      if analysis_graph[adj_node].level - analysis_graph[node].level < 5:
        wanted = False
        break
  if wanted:
    smm += mem

print("sum of wanted::"+str(smm/(1024*1024*1024)))
with open(out1, 'w') as f:
  for node in nodes_collocation_groups.keys():
      f.write(node + ' -1\n')
  for node in vanilla_placement:
    if node not in nodes_collocation_groups:
      f.write(node + ' ' +vanilla_placement[node] + '\n')
        ...
        if overflow > 0:
            ...
            for level in levels_indices_map.keys():
                ...
                for grpp in range(0, no_of_desired_groups):
                    ...
                    if final_groups_memory_consumptions[grpp][levels_indices_map[level]] > 32 * (1024 * 1024 * 1024):
                        prntt = True
