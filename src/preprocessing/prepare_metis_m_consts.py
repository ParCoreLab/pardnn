...
with open(out, 'w') as f:
    f.write( str(no_of_nodes) + " " + str(no_of_edges) + " " + "011 " + str(max_level) + "\n")
    for i in range(1, no_of_nodes + 1):
        if str(i) in rev_node_mapping:
            node = rev_node_mapping[str(i)]
            node_weight_vector = [0] * (max_level )
            tensor_size = 1
            src_node = False
            if node in nodes_props_dict and node in nodes_levels:
                    node_weight_vector[nodes_levels[node]] = nodes_props_dict[node].duration
            ...
            if node in graph.keys():
                ...
                f.write(str(node_weight_vector).replace('[','').replace(',',' ').replace(']',' ') + ' ')
                for adj_node in adj:
