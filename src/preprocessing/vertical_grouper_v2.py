...
with open(in1, 'r') as f:
    for line in f:
        ...
        if len(splits) > 1:
            ...
            all_nodes[splits[1]].children.append(splits[1])
...
while len(paths) > 0:
    ...
    current_path_weight = paths_weights.pop(0)
    this_path_joined = False
    if current_path[0] != 'src':
        common_node = analysis_graph[current_path[0]].parents[0]
...
selected_main_groups_count = 0

while selected_main_groups_count < no_of_desired_groups:
    current_main_group_len = 0
    for i in range(0, len(initial_groups)):
        if len(initial_groups[i]) >= current_main_group_len and not i in main_groups_indices:
            current_main_group_len = len(initial_groups[i])
            main_groups_indices[selected_main_groups_count] = i
    selected_main_groups_count = selected_main_groups_count + 1


#merging the groups
while len(initial_groups) > no_of_desired_groups:
    indx_min = 0
    while indx_min in main_groups_indices:
        indx_min = indx_min + 1
    min_group_weight = groups_weights[indx_min]
    for i in range(0, len(groups_weights)):
        if i in main_groups_indices:
            continue
    ...
    for j in range(0, no_of_desired_groups):
        if indx_min < main_groups_indices[j]:
