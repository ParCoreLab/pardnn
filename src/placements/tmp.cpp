                    ...
                    if (Node::NO_OP != nodes[adj_name].get_type() &&
                        ...
                        edges[node_name][adj_name] =
                            (int)((ceil(stod(splits[1]) * COMM_TRANSFER_RATE_RECIPROCAL)) +
                                    COMM_LATENCY);
                        if (nodes[node_name].get_type() == Node::VAR)
    ...
    while (!to_visit.empty()) {
        ...
        for (string adj_node_name : rev_adj_list[current_node_name]) {
            ...
            adj_node->set_bottom_level(
                    max(current_node->get_bottom_level() +
                        (with_weights ? adj_node->get_duration() : 1) + edge_weight,
...
void Graph::obtain_mapped_clusters() {
    ...
    if (1 || (int)(DoP + 1) >= 2 * K) {
        for (int itrr = 0; itrr < 2; itrr++) {
                    ...
                    while (i < clusters_weights_map.size()) {
                        ...
                        if (primary_indx != -1 &&
                            ...
                            /* else if (itrr == 0 && lc_ptr->get_indx(str) != -1)
                               ...
                               string str3 =
                               "generator_1/e3d-lstm_6/e3d2/hidden_1/moments/stopgradient"; cout
                               << least_load << "\t" << most_load << "\t" << unmapped_work <<
                            ...
                            for (int ik = 0; ik < K; ik++)
                            {
                            int current_sum = getSum(primary_trees[ik],
                            targeted_span.second) - getSum(primary_trees[ik],
                            targeted_span.first); cout << current_sum << "\t";
                            }
                            cout << "\n-------------------------------------\n";
                            } */
                        }
                                ...
                                while (cntt > 0) {
                                    ...
                                    while (i < clusters_weights_map.size()) {
                                        ...
                                        if (primary_indx != -1 &&
                                            ...
                                            // cout<<least_loaded_lc<<"\n";
                                            least_loaded_lc = primary_indx;
                                            int most_load = most_load_primary_in_span(primary_trees,
                                                    targeted_span);
                                            int unmapped_work = unmapped_work_in_span(
                                                    BITree, primary_trees, targeted_span);
                                            int all_work_in_the_span =
                                            ...
                                            if (all_work_in_the_span / K >=
                                                    the_work_of_the_target_primary_if_merged ||
                                                    min(most_load,
                                                        least_load + lc_ptr->get_weight() +
                                                        clusters_comms[lc_ptr][max_lc_prt] -
                                                        total_primaries_comm / K + avg_delay) >=
                                                    the_work_of_the_target_primary_if_merged ||
                                                    comm_in_the_span +
                                                    clusters_comms[lc_ptr][max_lc_prt] >=
                                                    max(most_load,
                                                ...
                                                merge_destination_ptr.push_back(max_lc_prt);
                                                merge_clusters(lc_ptr, merge_destination_ptr[0], true,
                                                        primary_trees[primary_indx],
                                                        BITree_comms, clusters_comms,
            ...
            // cout << secondary_clusters.size() << "\t" << cntts[0] << "\t" <<
            // cntts[1] << "\t" << cntts[2] << "\t" << cntts[3] << "\t" << cntts[4] <<
            // " are cntts \n";
        } // end main for
            ...
            while (ii < clusters_weights.size()) {
                ...
                for (int i = 0; i < K; i++) {
                    ...
                    primaries_coms_map.push_back(make_pair(
                                ...
                                1.0 /
                                (double)(getSum(primary_trees[i], targeted_span.second) -
    ...
    for (auto it : long_term_comms_map) {
        cout << levels_nodes[it.first][0]->get_level() << "\t"
            << it.second / (2 * K) << "\n";
    }
    ii = 0;
    while (ii <
        ...
        for (Node *node : nodes) {
            ...
            for (string adj_node_name : adj_list[node_name]) {
                ...
                if (node->get_part() == adj_node_part || visited_parts[adj_node_part] ||
                        (adj_node_part > K && node->get_part() > K)) {
                    continue;
                }
...
void Graph::assign_distant_vars_to_cpu() {
    ...
    for (int i = 0; i < vars_earliest_needed_levels.size(); i++) {
        ...
        if (vars_earliest_needed_levels[i].first >
