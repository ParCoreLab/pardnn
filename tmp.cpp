...
Graph Graph::create_and_annotate_graph()
{
  ...
  //g.calc_nodes_bottom_levels(false, true);
      ...
      if (nd != NULL and nd->get_type() != Node::NO_OP)
      {
        for (auto &it : edges[node_name])
        {
          ...
          if (Node::NO_OP != nodes[adj_name].get_type() && adj_name != snk_node_name)
          {
            ...
            if(nodes[node_name].get_type() == Node::VAR)total_comm += edges[node_name][adj_name];
          }
          else
          {
  ...
  while (!to_visit.empty())
  {
    ...
    for (string adj_node_name : rev_adj_list[current_node_name])
    {
      ...
      adj_node->set_bottom_level(
          max(
              current_node->get_bottom_level() + (with_weights ? adj_node->get_duration() : 1) + edge_weight,
...
void Graph::obtain_mapped_clusters()
{
  ...
  string str = "generator_3/e3d-lstm_7/e3d2/tanh_1";
    ...
    for (int itrr = 0; itrr < 2; itrr++)
    {
      ...
      while (cntt > 0)
      {
        ...
        for (int main_step = 5; main_step <= 10; main_step++)
        {
          ...
          while (i < clusters_weights_map.size())
          {
            ...
            if (primary_indx != -1 && clusters_comms_as_totals[lc_ptr] <= clusters_comms[lc_ptr][max_lc_prt] && clusters_comms[lc_ptr][max_lc_prt] > COMM_LATENCY)
            {
              ...
              {
                string str2 = "generator_2/e3d-lstm_7/e3d3/sigmoid_2";
                string str3 = "generator_1/e3d-lstm_6/e3d2/hidden_1/moments/stopgradient";
                cout << least_load << "\t" << most_load << "\t" << unmapped_work << "\t" << all_work_in_the_span << "\t" << clusters_comms_as_totals[lc_ptr] << "\t" << clusters_spans[lc_ptr].first << "\t" << clusters_spans[lc_ptr].second << "\t" << the_work_of_the_target_primary_if_merged << "\t" << get_node_by_name(str2)->get_top_level() << "\t" << get_node_by_name(str3)->get_top_level() << "\n";
                //lc_ptr->print_cluster();
                cout << "\n-------------------------------------\n";
                for (int ik = 0; ik < K; ik++)
                {
        ...
        if (cntt > 0 || itrr == 0)
        {
          ...
          if (itrr == 0 || REPLICATION_FACTOR != K) {
            obtain_clusters_comms(clusters_comms, clusters_comms_as_totals, clusters_comms_as_totals_with_primaries, secondary_clusters, true);
          }
          else if (itrr == 1) {
  ...
  for(auto it : long_term_comms_map){
    cout<<levels_nodes[it.first][0]->get_level()<<"\t"<<it.second / (2 * K)<<"\n";
  }
  ii = 0;
  ...
  for(auto it : levels_nodes){
    ...
    for(Node * node : nodes){
      ...
      for(string adj_node_name : adj_list[node_name]){
        Node* adj_node = get_node_by_name(adj_node_name);
        int adj_node_part = adj_node->get_part();
        if(node->get_part() == adj_node_part || visited_parts[adj_node_part] || (adj_node_part > K && node->get_part() > K)){
          continue;
        }
...
void Graph::assign_distant_vars_to_cpu()
{
  ...
  for (int i = 0; i < vars_earliest_needed_levels.size(); i++)
  {
    ...
    int comm = edges[node->get_name()][adj_list[node->get_name()][0]] * 10;
    if (vars_earliest_needed_levels[i].first > total_comm_cost_host_devices + comm)
    {
  ...
  cout << count_assigned_to_cpu << "\t are assigned to cpu\n";
}
...
void Graph::handle_memory()
{
  ...
  for (int i = 0; i < most_loaded_part; i++)
  {
    ...
    while (parts_ratios[i].first < 1 && most_loaded_part > i)
    {
      ...
      while (parts_ratios[most_loaded_part].first <= 1 && most_loaded_part > i)
      {
        most_loaded_part--;
      }
      cout<<"ratios after\t";
  for (int i = 0; i < K; i++)
  {
    cout<<parts_ratios[i].first<<","<<var_memroy_ratios[parts_ratios[i].second]<<","<<intermediate_memroy_ratios[parts_ratios[i].second]<<","<<parts_ratios[i].second<<"\t";
  }
  cout<<"\n";
    }
  }
