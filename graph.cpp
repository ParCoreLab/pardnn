    ...
    if (splits.size() == 2)
    {
      ...
      if (nd != NULL and nd->get_type() != Node::NO_OP)
      {
        for (auto &it : edges[node_name])
        {
          ...
          if (Node::NO_OP != nodes[adj_name].get_type() && adj_name != snk_node_name)
          {
            edges[node_name][adj_name] = (int)((ceil(stod(splits[1]) * COMM_TRANSFER_RATE_RECIPROCAL)) + COMM_LATENCY);
...
const vector<string> &Graph::get_keys() const
{
  return keys;
}
...
void Graph::top_sort()
{
  ...
  while (!to_visit.empty())
  {
    ...
    for (string adj_node : adj_list[current_node])
    {
      ...
      get_node_by_name(adj_node)->set_level(max(current_node_level + 1, get_node_by_name(adj_node)->get_level()));
    }
  }
}
...
void Graph::top_sort_reversed()
{
  ...
  {
    string current_node = to_visit.front();
    int current_node_level = get_node_by_name(current_node)->get_reversed_level();
    to_visit.pop();
    for (string adj_node : rev_adj_list[current_node])
    {
      ...
      get_node_by_name(adj_node)->set_reversed_level(max(current_node_level + 1, get_node_by_name(adj_node)->get_reversed_level()));
    }
  }
}
...
void Graph::fill_levels_nodes()
{
  ...
  for (string node_name : keys)
  {
    Node *node = get_node_by_name(node_name);
    int node_level = node->get_top_level();
    if (levels_comms_as_sums.find(levels_indices[node_level]) == levels_comms_as_sums.end())
    {
    ...
    for (string adj_node_name : adj_list[node_name])
    {
      ...
      if (adj_node->get_part() < K && node->get_part() < K && visited_parts[adj_node->get_part()] == 0 && adj_node->get_part() != node->get_part())
      {
        levels_comms_as_sums[levels_indices[node_level]] += edges[node_name][adj_node_name];
...
void Graph::calc_nodes_top_levels(bool is_clustered, bool with_weights)
{
  ...
  while (!to_visit.empty())
  {
    ...
    int current_node_level = get_node_by_name(current_node_name)->get_top_level();
    to_visit.pop();
    for (string adj_node_name : adj_list[current_node_name])
    {
      ...
      Node *adj_node = get_node_by_name(adj_node_name);
      int edge_weight = ((current_node->get_part() == -1 || current_node->get_part() != adj_node->get_part() || !is_clustered) ? get_edge_weight(current_node_name, adj_node_name) : 1);
      adj_node->set_top_level(
          max(
              current_node->get_top_level() + (with_weights ? current_node->get_duration() : 1) + edge_weight,
...
void Graph::calc_nodes_bottom_levels(bool is_clustered, bool with_weights)
{
  ...
  while (!to_visit.empty())
  {
    string current_node_name = to_visit.front();
    Node *current_node = get_node_by_name(current_node_name);
    int current_node_level = get_node_by_name(current_node_name)->get_bottom_level();
    to_visit.pop();
    for (string adj_node_name : rev_adj_list[current_node_name])
    {
      ...
      Node *adj_node = get_node_by_name(adj_node_name);
      int edge_weight = ((current_node->get_part() == -1 || current_node->get_part() != adj_node->get_part() || !is_clustered) ? get_edge_weight(adj_node_name, current_node_name) : 1);
      adj_node->set_bottom_level(
          max(
              current_node->get_bottom_level() + (adj_node->get_duration()) + edge_weight,
...
void Graph::calc_nodes_tmp_top_levels(const unordered_map<string, bool> &visited, unordered_map<string, int> &tmp_top_levels,
                                      vector<string> &free_nodes, unordered_map<string, int> tmp_nodes_in_degrees, bool clustered = false)
{
    ...
    while (!to_visit.empty())
    {
      ...
      for (string adj_node : adj_list[current_node_name])
      {
        if (visited.find(adj_node) == visited.end())
        {
          ...
          int edge_weight = (!clustered || current_node->get_part() != get_node_by_name(adj_node)->get_part()) ? get_edge_weight(current_node_name, adj_node) : 1;
          tmp_top_levels[adj_node] = max(
              tmp_top_levels[current_node_name] + current_node->get_duration() + edge_weight,
...
string Graph::calc_nodes_tmp_bottom_levels(const unordered_map<string, bool> &visited, unordered_map<string, int> &tmp_bottom_levels,
                                           vector<string> &free_nodes, unordered_map<string, int> tmp_nodes_out_degrees)
{
  ...
  for (string free_node_name : free_nodes)
  {
    ...
    while (!to_visit.empty())
    {
      ...
      for (string adj_node : rev_adj_list[current_node_name])
      {
        if (visited.find(adj_node) == visited.end())
        {
          tmp_bottom_levels[adj_node] = max(
              tmp_bottom_levels[current_node_name] + get_node_by_name(adj_node)->get_duration() + get_edge_weight(adj_node, current_node_name),
              tmp_bottom_levels[adj_node]);

          if (--tmp_nodes_out_degrees[adj_node] == 0)
          {
            to_visit.push(adj_node);
            if (visited.find(adj_node) == visited.end() && tmp_bottom_levels[adj_node] > max_weighted_level)
            {
...
string Graph::calc_nodes_tmp_weighted_level(const unordered_map<string, bool> &visited, unordered_map<string, int> &tmp_weighted_levels,
                                            vector<string> &free_nodes, vector<string> &rev_free_nodes, unordered_map<string, int> &tmp_nodes_in_degrees, unordered_map<string, int> &tmp_nodes_out_degrees)
{
  unordered_map<string, int> tmp_top_levels;
  unordered_map<string, int> tmp_bottom_levels;
  calc_nodes_tmp_top_levels(visited, tmp_top_levels, free_nodes, tmp_nodes_in_degrees);
  calc_nodes_tmp_bottom_levels(visited, tmp_bottom_levels, rev_free_nodes, tmp_nodes_out_degrees);
  ...
  for (string node : keys)
  {
    ...
    if (visited.find(node) == visited.end() && tmp_weighted_levels[node] > max_weighted_level)
    {
...
void Graph::obtain_linear_clusters()
{
  ...
  for (string node_name : keys)
  {
    tmp_bottom_levels[node_name] = get_node_by_name(node_name)->get_bottom_level();
  }

  priority_queue<pair<double, string>> free_nodes_pq;
  free_nodes_pq.push(make_pair(get_node_by_name(src_node_name)->get_bottom_level(), src_node_name));
  ...
  while (!free_nodes_pq.empty())
  {
    ...
    if (primary_clusters.size() == K || visited.find(current_node) != visited.end())
    {
      current_node = free_nodes_pq.top().second;
      while (!free_nodes_pq.empty() && visited.find(current_node) != visited.end())
      {
        free_nodes_pq.pop();
        current_node = free_nodes_pq.top().second;
      }
    ...
    if (part == K)
    {
      ...
      for (string node_name : keys)
      {
        ...
        {
          free_nodes_pq.push(make_pair(get_node_by_name(node_name)->get_weighted_level() +
                                           get_node_by_name(node_name)->get_reversed_level() / (double)(get_node_by_name(src_node_name)->get_reversed_level()),
                                       node_name));
        }
        tmp_bottom_levels[node_name] = get_node_by_name(node_name)->get_bottom_level();
      }
    ...
    while (visited.find(current_node) == visited.end())
    {
      ...
      //{if(--tmp_nodes_in_degrees[adj] == 0){free_nodes_pq.push(make_pair( tmp_weighted_levels[adj], adj) );}}
      ...
      for (string adj : adj_list[current_node])
      {
        if ((part >= K || visited.find(adj) == visited.end()) && tmp_bottom_levels[adj] > max_level)
        {
    ...
    if (current_cluster.length() > 0)
    {
      if (part < K)
      {
        for (string node_name : keys)
        {
          tmp_bottom_levels[node_name] = get_node_by_name(node_name)->get_duration();
        }
        current_node = calc_nodes_tmp_bottom_levels(visited, tmp_bottom_levels, rev_free_nodes, tmp_nodes_out_degrees);
        if (part == 0)
        {
...
void Graph::obtain_mapped_clusters()
{
  ...
  std::sort(secondary_clusters.begin(), secondary_clusters.end(), LinearCluster::sorting_criteria_weighted_level);
  ...
  int median_path_weighted_level = secondary_clusters[3 * secondary_clusters.size() / 4].get_src_node()->get_weighted_level();
  int max_weighted_level = secondary_clusters[0].get_src_node()->get_weighted_level();
  int median_cluster_weight = clusters_weights[clusters_weights.size() / 4].first;
  ...
  int *BITree = constructBITree(levels_weights_as_sums, levels_weights_as_sums.size());
  int *BITree_comms = constructBITree(levels_comms_as_sums, levels_comms_as_sums.size());
  int *primary_trees[K];
  for (int i = 0; i < K; i++)
  {
    primary_trees[i] = constructBITree(levels_weights[i], levels_weights[i].size());
  }

  unordered_map<LinearCluster *, unordered_map<LinearCluster *, int>> clusters_comms;
  ...
  obtain_clusters_comms(clusters_comms, clusters_comms_as_totals, clusters_comms_as_totals_with_primaries, secondary_clusters, true);
  ...
  for (int itrr = 0; itrr < 2; itrr++)
  {
    ...
    while (cntt > 0)
    {
      ...
      for (int main_step = 5; main_step <= 10; main_step++)
      {
        ...
        while (i < main_step * secondary_clusters.size() / 10)
        {
          ...
          {
            clusters_weights_map.push_back(make_pair(clusters_spans[lc_ptr].second - clusters_spans[lc_ptr].first,
                                                      i));
          }
          i++;
        }
        ...
        {
          LinearCluster *lc_ptr = &secondary_clusters[clusters_weights_map[i].second];
          i++;
          ...
          int total_primaries_comm = clusters_comms_as_totals_with_primaries[lc_ptr];
          if (primary_indx != -1 && clusters_comms_as_totals[lc_ptr] <= clusters_comms[lc_ptr][max_lc_prt] && clusters_comms[lc_ptr][max_lc_prt] > COMM_LATENCY)
          {
            pair<int, int> targeted_span = make_pair(levels_indices[clusters_spans[lc_ptr].first],
                                                      levels_indices[clusters_spans[lc_ptr].second]);
            int least_loaded_lc;
            int least_load = least_load_primary_in_span(primary_trees, targeted_span, least_loaded_lc);
            //cout<<least_loaded_lc<<"\n";
            least_loaded_lc = primary_indx;
            int most_load = most_load_primary_in_span(primary_trees, targeted_span);
            int unmapped_work = unmapped_work_in_span(BITree, primary_trees, targeted_span);
            int all_work_in_the_span = getSum(BITree, targeted_span.second) - getSum(BITree, targeted_span.first);
            int comm_in_the_span = 0; //getSum(BITree_comms, targeted_span.second) - getSum(BITree_comms, targeted_span.first);
            int the_work_of_the_target_primary_if_merged = lc_ptr->get_weight() +
                                                            getSum(primary_trees[primary_indx], targeted_span.second) -
                                                            getSum(primary_trees[primary_indx], targeted_span.first);
            int the_work_of_the_target_primary = getSum(primary_trees[primary_indx], targeted_span.second) -
                                                  getSum(primary_trees[primary_indx], targeted_span.first);

            int latest_parent_level = get_latest_src_parent(lc_ptr)->get_top_level();
            int avg_delay = getSum(primary_trees[least_loaded_lc],
                                    levels_indices[lc_ptr->get_src_node()->get_top_level()]) -
                            getSum(primary_trees[least_loaded_lc], levels_indices[latest_parent_level]);

            if (all_work_in_the_span / K >= the_work_of_the_target_primary_if_merged ||
                max(most_load, least_load + lc_ptr->get_weight() + clusters_comms[lc_ptr][max_lc_prt] + avg_delay + unmapped_work / K) >= the_work_of_the_target_primary_if_merged ||
                the_work_of_the_target_primary_if_merged <= lc_ptr->get_weight() + clusters_comms[lc_ptr][max_lc_prt] ||
                clusters_comms[lc_ptr][max_lc_prt] >= most_load - least_load)
            {
              ...
              merge_clusters(lc_ptr, merge_destination_ptr[0], true, primary_trees[primary_indx], BITree_comms, clusters_comms, clusters_comms_as_totals, clusters_comms_as_totals_with_primaries);
            ...
            else if (lc_ptr->get_indx(str) != -1)
            {
              ...
              cout << least_load << "\t" << most_load << "\t" << unmapped_work << "\t" << all_work_in_the_span << "\t" << clusters_comms_as_totals[lc_ptr] << "\t" << clusters_spans[lc_ptr].first << "\t" << 
              clusters_spans[lc_ptr].second << "\t" << the_work_of_the_target_primary_if_merged << "\t" << get_node_by_name(str2)->get_top_level() << "\t" << get_node_by_name(str3)->get_top_level() << "\n";
              //lc_ptr->print_cluster();
              cout << "\n-----------------1--------------------\n";
              for (int ik = 0; ik < K; ik++)
              {
                int current_sum = getSum(primary_trees[ik], targeted_span.second) - getSum(primary_trees[ik], targeted_span.first);
                cout << current_sum << "\t";
              }
              cout << "\n------------------1-------------------\n";
            }
        ...
        if (K == REPLICATION_FACTOR || ceil(CCR) >= max(10, 2 * (int)DoP))
        {
          while (i < clusters_weights_map.size())
          {
            ...
            int total_primaries_comm = clusters_comms_as_totals_with_primaries[lc_ptr];
            if (total_primaries_comm <= clusters_comms[lc_ptr][max_lc_prt] && clusters_comms[lc_ptr][max_lc_prt] > COMM_LATENCY)
            {
              if (primary_indx != -1)
              {
                pair<int, int> targeted_span = make_pair(levels_indices[clusters_spans[lc_ptr].first],
                ...
                {
                  string str2 = "generator_2/e3d-lstm_10/e3d2/inputs_1/moments/stopgradient";
                  string str3 = "generator_3/e3d-lstm_10/e3d2/inputs_1/moments/stopgradient";
                  cout << least_load << "\t" << most_load << "\t" << unmapped_work << "\t" << all_work_in_the_span << "\t" << clusters_comms_as_totals[lc_ptr] << "\t" << clusters_spans[lc_ptr].first << "\t" << clusters_spans[lc_ptr].second << 
                  "\t" << the_work_of_the_target_primary_if_merged << "\t" << get_node_by_name(str2)->get_top_level() << "\t" << get_node_by_name(str3)->get_top_level() << "\n";
                  //lc_ptr->print_cluster();
                  cout << "\n-----------------2--------------------\n";
                  for (int ik = 0; ik < K; ik++)
                  {
      ...
      if (cntt > 0 || itrr == 0)
      {
        ...
        BITree = constructBITree(levels_weights_as_sums, levels_weights_as_sums.size());
        BITree_comms = constructBITree(levels_comms_as_sums, levels_comms_as_sums.size());
        for (int i = 0; i < K; i++)
        {
          primary_trees[i] = constructBITree(levels_weights[i], levels_weights[i].size());
        }*/

        if (itrr == 1 && (REPLICATION_FACTOR == K || ceil(CCR) >= max(10, 2 * (int)DoP)) )
        {
          obtain_clusters_comms(clusters_comms, clusters_comms_as_totals, clusters_comms_as_totals_with_primaries, secondary_clusters, false);
        }
        else
        {
          obtain_clusters_comms(clusters_comms, clusters_comms_as_totals, clusters_comms_as_totals_with_primaries, secondary_clusters, true);
        ...
        cout << "cnttttttttttttttttttttttttttttttttttttttt is\t" << cntt << "\n";
      }
      else
      {
        ...
        if (K == REPLICATION_FACTOR || ceil(CCR) >= max(10, 2 * (int)DoP))
        {
          for (int main_step = 5; main_step <= 10; main_step++)
          {
            ...
            while (i < main_step * secondary_clusters.size() / 10)
            {
              ...
              {
                clusters_weights_map.push_back(make_pair(get_earliest_snk_child(lc_ptr)->get_level() - lc_ptr->get_src_node()->get_level(),
                                                          i));
              }
              i++;
            }
            ...
            for (int step = 1; step <= 20; step++)
            {
              ...
              while (cntt > 0)
              {
                ...
                while (i < clusters_weights_map.size())
                {
                  ...
                  bool prnt = false;
                  int total_primaries_comm = clusters_comms_as_totals_with_primaries[lc_ptr];
                  if (primary_indx != -1 && clusters_comms[lc_ptr][max_lc_prt] > COMM_LATENCY && clusters_comms[lc_ptr][max_lc_prt] > max(total_primaries_comm / 2, clusters_comms_as_totals[lc_ptr] / K))
                  {
                    pair<int, int> targeted_span = make_pair(levels_indices[clusters_spans[lc_ptr].first],
                                                              levels_indices[clusters_spans[lc_ptr].second]);
                    int least_loaded_lc;
                    ...
                    if (all_work_in_the_span / K >= the_work_of_the_target_primary_if_merged ||
                        min(most_load, least_load + lc_ptr->get_weight() + clusters_comms[lc_ptr][max_lc_prt] - total_primaries_comm / K + avg_delay) >= the_work_of_the_target_primary_if_merged ||
                        comm_in_the_span + clusters_comms[lc_ptr][max_lc_prt] >= max(most_load, the_work_of_the_target_primary_if_merged))
                    {
                      vector<LinearCluster *> merge_destination_ptr;
                      merge_destination_ptr.push_back(max_lc_prt);
                      merge_clusters(lc_ptr, merge_destination_ptr[0], true, primary_trees[primary_indx], BITree_comms, clusters_comms, clusters_comms_as_totals, clusters_comms_as_totals_with_primaries);
    ...
    cout << secondary_clusters.size() << "\t" << cntts[0] << "\t" << cntts[1] << "\t" << cntts[2] << "\t" << cntts[3] << "\t" << cntts[4] << " are cntts \n";
  } //end main for
  ...
  if (REPLICATION_FACTOR == K || ceil(CCR) >= max(10, 2 * (int)DoP))
  {
    while (ii < clusters_weights.size())
    {
      ...
      for (int i = 0; i < K; i++)
      {
        ...
        primaries_coms_map.push_back(make_pair(clusters_comms[lc_ptr][primary_ptr] +
                                                    1.0 / (double)(getSum(primary_trees[i], targeted_span.second) - getSum(primary_trees[i], targeted_span.first) + 1.0),
                                                i));
      }
      primaries_coms_map.push_back(make_pair(-1, primaries_coms_map[K - 1].first));
      std::sort(primaries_coms_map.rbegin(), primaries_coms_map.rend());
      int max_comm = clusters_comms[lc_ptr][&primary_clusters[primaries_coms_map[0].second]];
      int comm_in_the_span = 0; //getSum(BITree_comms, targeted_span.second) - getSum(BITree_comms, targeted_span.first);
      int j = 0;
      while (clusters_comms[lc_ptr][&primary_clusters[primaries_coms_map[j].second]] > COMM_LATENCY)
      {
        int the_work_of_the_target_primary_if_merged = lc_ptr->get_weight() + getSum(primary_trees[i], targeted_span.second) - getSum(primary_trees[i], targeted_span.first);
        int i = primaries_coms_map[j].second;
        j++;
        int curret_comm = clusters_comms[lc_ptr][&primary_clusters[i]];
        if (min(most_load - comm_in_the_span, least_load + lc_ptr->get_weight() + curret_comm + (curret_comm - max_comm)) >= the_work_of_the_target_primary_if_merged ||
            the_work_of_the_target_primary_if_merged + comm_in_the_span <= most_load ||
            the_work_of_the_target_primary_if_merged <= lc_ptr->get_weight() + curret_comm ||
            comm_in_the_span + curret_comm >= max(most_load, the_work_of_the_target_primary_if_merged))
        {
          vector<LinearCluster *> merge_destination_ptr;
          merge_destination_ptr.push_back(&primary_clusters[i]);
          merge_clusters(lc_ptr, merge_destination_ptr[0], true, primary_trees[i], BITree_comms, clusters_comms, clusters_comms_as_totals, clusters_comms_as_totals_with_primaries);
          merged_clusters[lc_ptr] = merge_destination_ptr[0];
          cntt++;
          cntts[0]++;
          cntts[4]++;
  ...
  obtain_clusters_comms(clusters_comms, clusters_comms_as_totals, clusters_comms_as_totals_with_primaries, secondary_clusters, false);
    ii = 0;
  while (ii < clusters_weights.size()) // && clusters_weights[ii].first >= weight_threshold - weight_threshold_step)
  {
    ...
    int min_cost = getSum(primary_trees[0], levels_indices[clusters_spans[lc_ptr].second]) - getSum(primary_trees[0], levels_indices[clusters_spans[lc_ptr].first]) +
                   -clusters_comms[lc_ptr][&primary_clusters[0]];
    int min_indx = 0;
    for (int j = 1; j < primary_clusters.size(); j++)
    {
      int current_cost = getSum(primary_trees[j], levels_indices[clusters_spans[lc_ptr].second]) - getSum(primary_trees[j], levels_indices[clusters_spans[lc_ptr].first]) +
    ...
    merge_clusters(lc_ptr, merge_destination_ptr[0], true, primary_trees[min_indx], BITree_comms, clusters_comms, clusters_comms_as_totals, clusters_comms_as_totals_with_primaries);
    merged_clusters[lc_ptr] = merge_destination_ptr[0];
  }
  counts[K] = {0};
  for (string node : keys)
...
void Graph::merge_clusters(LinearCluster *src, LinearCluster *dst, bool merge_work, int *BITree, int *BITree_comms, unordered_map<LinearCluster *, unordered_map<LinearCluster *, int>> &clusters_comms,
                           unordered_map<LinearCluster *, int> &clusters_comms_as_totals, unordered_map<LinearCluster *, int> &clusters_comms_as_totals_with_primaries)
{
  ...
  for (Node *node : src->nodes)
  {
    if (merge_work)
    {
      updateBIT(BITree, levels_weights_as_sums.size(), levels_indices[node->get_top_level()], node->get_duration());
    }
    ...
    for(string adj_name: adj_list[node_name]){
      ...
      LinearCluster *adj_lc = adj_node_part < K ? &primary_clusters[adj_node_part] : &secondary_clusters[adj_node_part - K];
      if (visited_parts.find(adj_lc) == visited_parts.end())
      {
        visited_parts[adj_lc] = 0;
      }
      visited_parts[adj_lc] = max(visited_parts[adj_lc], edges[node_name][adj_name]);  
    }
    for(string rev_adj_name: rev_adj_list[node_name]){
      ...
      visited_parts[adj_lc] = max(visited_parts[adj_lc], edges[node_name][rev_adj_name]);
    }
  ...
  if (dst->nodes[old_length]->get_top_level() < dst->nodes[0]->get_top_level())
  {
    Node *tmp = dst->nodes[0];
    dst->nodes[0] = dst->nodes[old_length];
    dst->nodes[old_length] = tmp;
  }
  if (dst->nodes[old_length - 1]->get_top_level() > dst->nodes[dst->length() - 1]->get_top_level())
  {
...
LinearCluster *Graph::max_communicating(LinearCluster *lc, unordered_map<LinearCluster *, unordered_map<LinearCluster *, int>> &clusters_comms)
{
  int max_comm = -1;
...
void Graph::obtain_clusters_comms(unordered_map<LinearCluster *, unordered_map<LinearCluster *, int>> &clusters_comms,
                                  unordered_map<LinearCluster *, int> &clusters_comms_as_totals, unordered_map<LinearCluster *, int> &clusters_comms_as_totals_with_primaries, vector<LinearCluster> &target_clusters,
                                  bool is_critical)
{
  ...
  for (string node_name : keys)
  {
    ...
    /* for (string adj_node_name : adj_list[node_name])
    {
      ...
      if (adj_part < K)
      {
        nodes_primaries_comms[node][adj_part] = max(nodes_primaries_comms[node][adj_part], edges[node_name][adj_node_name]);
      }
    } */
  }
  ...
  for (int i = 0; i < target_clusters.size(); i++)
  {
    ...
    for (Node *node : lc_ptr->nodes)
    {
      ...
      for (string adj_name : adj_list[node_name])
      {
        ...
        {
          LinearCluster *target_cluster = adj_part < K ? &primary_clusters[adj_part] : (&secondary_clusters[adj_part - K]);

          if (clusters_comms[lc_ptr].find(target_cluster) == clusters_comms[lc_ptr].end())
          {
            clusters_comms[lc_ptr][target_cluster] = 0;
          }
          if (clusters_comms[target_cluster].find(lc_ptr) == clusters_comms[target_cluster].end())
          {
          ...
          visited_parts[target_cluster] = max(visited_parts[target_cluster], comm);
        }
      }
...
void Graph::obtain_clusters_spans(unordered_map<LinearCluster *, pair<int, int>> &clusters_spans, vector<LinearCluster> &target_clusters)
{
  for (int i = 0; i < target_clusters.size(); i++)
  {
    LinearCluster *lc_ptr = &target_clusters[i];
    int latest_parent_level = lc_ptr->get_src_node()->get_top_level(), earliest_child_level = get_node_by_name(snk_node_name)->get_top_level();
    ...
    for (string adj_name : rev_adj_list[lc_ptr->get_src_node()->get_name()])
    {
      ...
      int rev_adj_earliest_possible_child = get_node_by_name(adj_name)->get_top_level() + get_node_by_name(adj_name)->get_duration();
      int start_indx = levels_indices[get_node_by_name(adj_name)->get_top_level()];
      bool found = false;
      ...
      if (levels_indices.find(rev_adj_earliest_possible_child) != levels_indices.end() && rev_adj_earliest_possible_child < latest_parent_level)
      {
        latest_parent_level = rev_adj_earliest_possible_child;
      }
    }
    lc_ptr->set_span(make_pair(latest_parent_level, earliest_child_level));
    clusters_spans[lc_ptr] = make_pair(latest_parent_level, earliest_child_level);
  }
}
...
void Graph::collocate()
{
  ...
  /* for (string node_name : keys)
  {
    if (get_node_by_name(node_name)->get_part() == -1 || (get_node_by_name(node_name)->get_type() != Node::VAR && !has_suffix(node_name, "/read")) )
    {
      continue;
    }
  ...
  for (string node_name : keys)
  {
    if (get_node_by_name(node_name)->get_type() != Node::VAR || changed.find(node_name) != changed.end())
    {
      continue;
    }
    ...
    /* if(!assigned_to_cpu){
      for (string adj_node_name : adj_list[node_name])
      {
        ...
        if (get_node_by_name(adj_node_name)->get_type() == Node::REF_OP)
        {
          ...
          parts_freqs[adj_part] += get_node_by_name(adj_node_name)->get_weighted_level();
          if (parts_freqs[adj_part] > max_freq)
          {
      ...
      for (string adj : adj_list[current_node_name])
      {
        ...
        if (adj_node->get_type() == Node::REF_OP)
        {
          ...
          for (string rev_adj : rev_adj_list[adj])
          {
            ...
            if (changed.find(rev_adj) == changed.end() && rev_adj_node->get_type() == Node::VAR)
            {
...
int Graph::most_load_primary_in_span(int *primary_trees[], pair<int, int> span)
{
  int max_load = 0;
  int lc_indx;
  for (int i = 0; i < K; i++)
  {
    int current_sum = getSum(primary_trees[i], span.second) - getSum(primary_trees[i], span.first);
...
int Graph::least_load_primary_in_span(int *primary_trees[], pair<int, int> span, int &lc)
{
  int min_load = getSum(primary_trees[0], span.second) - getSum(primary_trees[0], span.first);
  int lc_indx = 0;
  for (int i = 1; i < K; i++)
  {
    int current_sum = getSum(primary_trees[i], span.second) - getSum(primary_trees[i], span.first);
...
int Graph::unmapped_work_in_span(int *BITree, int *primary_trees[], pair<int, int> span)
{
  int mapped_sum = 0;
  for (int i = 0; i < K; i++)
  {
    mapped_sum += getSum(primary_trees[i], span.second) - getSum(primary_trees[i], span.first);
  }
...
void Graph::place_cpu_nodes()
{
  ...
  while (getline(my_file, line))
  {
    ...
    if (splits.size() == 2 &&
        splits[1] == "-1" &&
        get_node_by_name(splits[0])
        //&& ( (get_node_by_name(splits[0])->get_level() < 500 || get_node_by_name(splits[0])->get_level() > 2000) || splits[1] == "-1")
    )
    {
...
int Graph::max_communicating_primary(LinearCluster *lc_ptr, unordered_map<LinearCluster *, unordered_map<LinearCluster *, int>> &clusters_comms)
{
  int primary_indx = -1;
  int max_comm = 0;
  for (int j = 0; j < K; j++)
...
int Graph::get_makespan()
{
  ...
  while (!to_visit.empty())
  {
    ...
    {
      Node *adj_node = get_node_by_name(adj_node_name);
      int edge_weight = current_node->get_part() != adj_node->get_part() ? get_edge_weight(current_node_name, adj_node_name) : 1;
      adj_node->set_top_level(
          max(
              parts_ready_times[current_node->get_part()] + edge_weight,
              parts_ready_times[adj_node->get_part()]));
      if (--tmp_nodes_in_degrees[adj_node_name] == 0)
      {
        to_visit.push(adj_node_name);
        parts_ready_times[adj_node->get_part()] = adj_node->get_top_level() + adj_node->get_duration();
      }
    }
  }
...
map<int, int> Graph::comm_to_primary(Node *nd, LinearCluster *lc)
{
  ...
  for (string adj_node : adj_list[nd->get_name()])
  {
    ...
    {
      comms[levels_indices[nd->get_top_level()]] = max(comms[levels_indices[nd->get_top_level()]], edges[nd->get_name()][adj_node]);
    }
  }
  for (string rev_adj_node : rev_adj_list[nd->get_name()])
  {
    if (comms.find(levels_indices[get_node_by_name(rev_adj_node)->get_level()]) == comms.end())
    {
      comms[levels_indices[levels_indices[get_node_by_name(rev_adj_node)->get_level()]]] = 0;
    }
    if (&primary_clusters[get_node_by_name(rev_adj_node)->get_part()] == lc)
    {
      comms[levels_indices[levels_indices[get_node_by_name(rev_adj_node)->get_level()]]] += comms[levels_indices[levels_indices[get_node_by_name(rev_adj_node)->get_level()]]];
    }
  }
  return comms;
}
...
void Graph::assign_distant_vars_to_cpu(int heavy_comm_last_level)
{
  ...
  for (string node_name : keys)
  {
    ...
    vars_earliest_needed_levels.push_back(make_pair(min_level_child, get_node_by_name(node_name)));
  }
  std::sort(vars_earliest_needed_levels.begin(), vars_earliest_needed_levels.end());
  ...
  for (int i = 0; i < vars_earliest_needed_levels.size(); i++)
  {
    ...
    if (vars_earliest_needed_levels[i].first - heavy_comm_last_level > (total_comm_cost_host_devices + comm)/K)
    {
  ...
  cout << count_assigned_to_cpu << "\t are assigned to cpu with memory \t"<<mem_assigned_to_cpu<<"\n";
}
...
void Graph::assign_heavy_memory_consumers_to_cpu(){
  for(string node_name : keys){
    ...
    if(node_level - earliest_child_level >= get_node_by_name(snk_node_name)->get_level() / 2 && reserved_for >= (0.1 * DEVICE_MEM_CAPACITY)){
...
void Graph::handle_memory()
{
  ...
  for (int i = 0; i < K; i++)
  {
    intermediate_memroy_ratios[i] = total_intermediate_memories[i] / total_intermediate_memory;
    var_memroy_ratios[i] = total_var_memories[i] / total_var_memory;
  }
  for (int i = 0; i < K; i++)
  {
    parts_ratios.push_back(make_pair(K * (intermediate_memroy_ratios[i] + var_memroy_ratios[i]) / 2.0, i));
  }

  cout << "ratios before\t";
  for (int i = 0; i < K; i++)
  {
    cout << parts_ratios[i].first << "," << var_memroy_ratios[parts_ratios[i].second] << "," << intermediate_memroy_ratios[parts_ratios[i].second] << "," <<
  ...
  for (int i = 0; i < most_loaded_part; i++)
  {
    ...
    while (parts_ratios[i].first < 1 && most_loaded_part > i && parts_ratios[i].first < parts_ratios[most_loaded_part].first)
    {
      ...
      var_memroy_ratios[part_indx] += node_to_move->get_memory() / total_var_memory;
      var_memroy_ratios[most_loaded_part_index] -= node_to_move->get_memory() / total_var_memory;
      parts_ratios[i].first = K * (intermediate_memroy_ratios[part_indx] + var_memroy_ratios[part_indx]) / 2.0;
      parts_ratios[most_loaded_part].first = K * (intermediate_memroy_ratios[most_loaded_part_index] + var_memroy_ratios[most_loaded_part_index]) / 2.0;
      while ((parts_ratios[most_loaded_part].first <= 1 || parts_var_nodes[most_loaded_part_index].size() == 0) && most_loaded_part > i)
      {
        most_loaded_part--;
